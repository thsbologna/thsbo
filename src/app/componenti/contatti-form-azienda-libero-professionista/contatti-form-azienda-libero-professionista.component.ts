...
@Component({
  selector: 'app-contatti-form-azienda-libero-professionista',
  standalone: true,
  ...
  templateUrl: './contatti-form-azienda-libero-professionista.component.html',
  styleUrl: './contatti-form-azienda-libero-professionista.component.css'
})
export class ContattiFormAziendaLiberoProfessionistaComponent {
  form!: FormGroup;
  formValido!: boolean;
  ...
  private creaForm() {
    this.form = this.formBuilder.group({
      ...
      partitaIVA: [null, [Validators.required, Validators.minLength(11), Validators.maxLength(11), Validators.pattern(this.regexPatterns.partitaIVA)]],
  ...
  onSubmit() {
    ...
    if (this.form.valid) {
      ...
      this.email.tipoAttivita = this.email.tipoAttivita.replaceAll("/", '_').replaceAll(" ", "_");
      this.email.prefisso = this.email.prefisso.valueOf();
      this.emailService.inviaEmailAziendaLiberoProfessionista(this.email).subscribe({
        next: () => {
